...
Conclusion: It begins with 1 billion coins. It ends with one world, held firm under Sphincter Coin.`
  ...
  return (
    ...
    <header className="p-4 mb-12 sm:mb-5">
      ...
      <a href="/" target="_blank" rel="noopener noreferrer" title='sphincter coin - Farts from Buttholes can’t happen without a Sphincter.' >
        ...
        <span className="text-xs font-light item-start ml-3">copus-tutor &gt;</span>
        <span className="text-xs font-light item-start ml-3">First choice! Sphincter <span className='bg-gold text-black'>coin</span> . After all a fart cannot exist without a butthole</span>
        <span className="text-xs font-light item-start ml-3">and a butthole cannot function without a sphincter.</span>
        <span className="text-xs font-light item-start ml-3">...</span>
        <img src="./images/sphincter-logo.png" alt="sphincter Coin Logo" width={200} height={200} className="mr-2" />
        <span className="text-xs font-light ml-3">...</span>
        <span className="text-xs font-light ml-3">Initializing Hole</span>
        <span className="text-xs font-light ml-3">Because Even Buttholes Need Boundaries.</span>
        </div>
        </div>
      </a>
    </header>
      ...
      <div className="space-y-4 mb-8">
        {phases.slice(0, currentPhase + 1).map((phase, index) => (
          <TerminalText
            ...
            text={phase}
            speed={10}
            onComplete={index === currentPhase ? handlePhaseComplete : undefined}
          />
        ))}
      </div>
    </main>
    ...
    <footer className="p-6 border-t border-gray-800 mt-20">
      <div className="max-w-4xl mx-auto flex flex-wrap justify-center items-center gap-8">
        <a href="https://www.pumpdotfun.com" target="_blank" rel="noopener noreferrer" className="hover:opacity-75 transition-opacity">
          <img src="./images/pump-fun-logo.png" alt="pumpdotfun" width={120} height={40} className="h-8 w-auto" />
        </a>
